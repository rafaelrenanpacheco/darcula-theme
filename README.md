...
* Try to keep them sorted in ascending order: `ctrl + shift + p` + `Sort Lines Ascending`.
* Submit a PR.

In order to identify the `scope` of a token, first go to the file that you are using as reference. Then press `ctrl + shift + p`, search and execute `Developer: Inspect Editor Tokens and Scopes`. Now click on the token that you wish to customize and the inspector will show you the scopes related to that token.
